...
#BASICAMENTE É UM SENDTO NORMAL
#O RESTO É INÚTIL PARA A GNT
...
#É UM RECVFROM NORMAL, MAS O TAMANHO DO BUFFER É PASSADO COMO ARGUMENTO
...
#CALCULA O CHECKSUM DA MENSAGEM PASSADA EM BYTES
...
#BASICAMENTE CRIA O SOCKET NORMALMENTE
def rdt_socket():
...
#BASICAMENTE DÁ UM BIND DO SOCKET NA SUA RESPECTIVA PORTA
...
#CRIA O ENDEREÇO DO PEER -> (IP, PORTA)
#ISSO É O NOSSO self.addr
...
#CRIA O PACOTE QUE VAI SER ENVIADO PARA O OUTRO LADO
def __make_data(seq_num, data):
    """Make DATA [seq_num].

    Input arguments: sequence number, data
...
#VERIFICA SE O PACOTE RECEBIDO ESTÁ OU NÃO CORROMPIDO
...
#GARANTE QUE O TAMANHO DOD DADOS É MENOR QUE O PAYLOAD
...
def rdt_send(sockd, peer_addr, byte_msg):
    ...
    """
    global PAYLOAD, __data_buffer, HEADER_SIZE, __send_seq_num, __last_ack_no
    ...
    #ENVIA A MENSAGEM PARA O OUTRO LADO
    # Try to send packet
    try:
        sent_len = __udt_send(sockd, peer_addr, snd_pkt)
    except socket.error as err_msg:
    ...
    #FICA ESPERANDO O ACK DO OUTRO LADO OU UM TIMEOUT
        ...
        if r:  # ACK (or DATA) came
            for sock in r:
                ...
                # Received *complete* DATA while waiting for ACK
                    ...
                    try:
                        __udt_send(sockd, peer_addr, __make_ack(data_seq_num))
                    except socket.error as err_msg:
...
#FICA ESPERANDO UMA MENSAGEM RDT DO OUTRO LADO
def rdt_recv(sockd, peer_addr, length):
    ...
    Note: Catch any known error and report to the user.
    """
    global __data_buffer, __recv_seq_num, HEADER_SIZE, __last_ack_no
    ...
    while not recv_expected_data:  # Repeat until received expected DATA
        ...
        if __is_corrupt(recv_pkt) or __has_seq(recv_pkt, 1-__recv_seq_num):
            ...
            try:
                __udt_send(sockd, peer_addr, snd_ack)
            except socket.error as err_msg:
        ...
        # If received DATA with *expected* seq num, send ACK
            ...
            # Send right ACK
            try:
                __udt_send(sockd, peer_addr, __make_ack(__recv_seq_num))
            except socket.error as err_msg:
...
#VERIFICA SE O PACOTE RECEBIDO TEM SEQ ESPERADA
